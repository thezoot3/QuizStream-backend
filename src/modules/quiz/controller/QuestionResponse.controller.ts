...
export class QuestionResponseController {
  ...
  @Get('/quiz/:id/user/:userId')
  async findByQuizAndUser(@Param('id') id: string, @Param('userId') userId: string) {
    return this.questionResponseService.findByQuizAndUserId(id, userId);
  }
